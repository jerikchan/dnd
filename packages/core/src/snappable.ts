...
function groupByElementGuidelines(
  ...
  const group = groupBy(guidelines, ({ element, pos }) => {
...
function getNearestSnapGuidelineInfo(snapInfo: SnapInfo) {
  ...
  function renderSizeValue(
    ...
    group.forEach((elementGuidelines, i) => {
      elementGuidelines.forEach(({ pos, size }) => {
        const { pos: linePos, size: lineSize } = getElementGuidelineDist(
          pos[index],
          size,
          clientPos,
          clientSize
